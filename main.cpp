...
void DrawFloor (Model* model, Shader* shader);
void DrawWashBasin(Model* model, Shader* shader);
void DrawDead(Model* model, Shader* shader);
void DrawAll();
...
bool drawFloorOnce = true; 
bool drawDeadPoolOnce = true; 
GLfloat deltaTime = 30.0f;
...
int main()
{
	...
	// Load models
	Model deadpool("../Source/dead 123456.obj"); 
	Model floor("../Source/Cobblestones3/untitled.obj");
	Model ball("../Source/Ball OBJ.obj");
	Model wash_basin("../Source/Lavabo.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glfwPollEvents();
		Do_Movement();
		DrawHere(&ball, &shader);
		DrawFloor (&floor, &shader);
		DrawDead(&deadpool, &shader); 
...
void DrawHere(Model* model, Shader* shader)
{
	if (key1[GLFW_KEY_ENTER])
	{
		...
		thingz.push_back(new Thing(model, shader, camera, 10.0f, 2.00));
	}
}

void DrawFloor(Model* model, Shader* shader)
{	
	if (drawFloorOnce == true)
	{
		thingz.push_back(new Thing(model, shader, camera, 60.0f, 10.00f));
		drawFloorOnce = false;
	}
}
...
void DrawDead(Model* model, Shader* shader)
{
	if (drawDeadPoolOnce == true)
	{
		thingz.push_back(new Thing(model, shader, camera, 60.0f, 0.1f));
		drawDeadPoolOnce = false;
	}
}
